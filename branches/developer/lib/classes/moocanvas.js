...
var Canvas = new Class({
	...
	initialize: function(id, props) {
		...
		if($type(id) == 'string') {
			...
			if(!el.getContext) {
				...
				el.attachEvent('onpropertychange', this.onPropertyChange);
				el.attachEvent('onresize', this.onResize);
			}
		}
		el=$extend(el,new CanvasExtenders());
		return el;
	},
	...
	onPropertyChange: function(e) {
		...
		switch (e.propertyName) {
			case 'width': case 'height': 
				el.setStyle(e.propertyName, el.getAttribute('height'));
	...
	onResize: function(e) {
		e = e.srcElement;
		if (e.firstChild) e.setStyles({ width: e.clientWidth, height: e.clientHeight });
	}
...
CanvasRenderingContext2D.implement({	
	...
	*/
	restore: function() {
		this.copyState(this.aStack.pop());
		this.m = this.mStack.pop();
	},
	
	copyState: function(obj) {
...
CanvasRenderingContext2D.implement({
	...
	createPattern: function(img, rep) {
		//rep='repeat','repeat-x','repeat-y','no-repeat'
...
var CanvasExtenders = new Class({
	...
	roundedRectangle: function (x,y,width,height,radius){
		...
		ctx.lineTo(x,height-radius+y); ctx.quadraticCurveTo(x,height+y,radius+x,height+y);
		ctx.lineTo(width-radius+x,height+y); ctx.quadraticCurveTo(width+x,height+y,width+x,height-radius+y);
		ctx.lineTo(width+x,radius+y); ctx.quadraticCurveTo(width+x,y,width-radius+x,y);	
		ctx.lineTo(radius+x,y); ctx.quadraticCurveTo(x,y,x,radius+y);
		ctx.closePath();
	}
});
