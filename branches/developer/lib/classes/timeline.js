	...
	drawConnection:function(from,to){
		if(from){
			...
			if (!from.hasClass("projectBox")) {
				...
				toM = toPos.top + (toPos.height / 2);
			}else {
				fromM = fromPos.top + (this.options.style.projectTitleEight / 2);
				toM = toPos.top + (this.options.style.projectTitleEight / 2);
			}
...
TimeLine.Resource=new Class({
	...
	drawTasks:function(){
		...
		this.resource.tasks.each(function(task){
			...
			var yin=projPos.y+this.project.timeline.options.style.projectTitleEight+this.resource.position-2;
