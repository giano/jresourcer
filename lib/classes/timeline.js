...
var TimeLine=new Class({
	...
	initialize:function(title,options){
		 ...
		 this.canvasbk=new Canvas(this.options.containerId+"_canvasbk",sx).injectInside($(this.options.containerId))
		 this.canvasbk.setStyles({
			'position':'absolute',
			'top':'0px',
			'left':'0px',
			'z-index':'-1',
		 ...
		 this.text=new Element("div", {
		 	id: this.options.containerId + "_text"
		 }).injectInside($(this.options.containerId))
		 this.text.setStyles({
			'position':'absolute',
			'top':'0px',
	...
	initCanvas:function(){
		...
		this.canvasbk.setProperty("width",sx.x);
		this.canvasbk.setProperty("height",sx.y);
		this.canvasbk.setStyles({
			'width':sx.x+'px',
			'height':sx.y+'px'
		});
		this.ctxbk.clearRect(0,0,sx.x,sx.y);
		this.ctxbk.lineCapp  = "butt";
	},
	drawMonths:function(){
		var sx=this.boxes.getSize().scrollSize;
		...
		for(var k=0;k<sx.x;k+=this.options.style.dayWidth){
			...
			if(dtn.getDate()==1){
				...
				this.ctxbk.strokeStyle = "rgba(0, 0, 0,.2)";
			    this.ctxbk.lineWidth  = 1;
				this.ctxbk.moveTo(k,0);
				this.ctxbk.lineTo(k,sx.y);
				this.ctxbk.stroke();
			}
			date+=one_day;
		}
	...
	drawWeeks:function(){
		...
		for(var k=0;k<sx.x;k+=this.options.style.dayWidth){
			...
			if(dt.getDay()==0){
				...
				this.ctx.fill();
				this.ctxbk.strokeStyle = "rgba(0, 0, 0,.1)";
			    this.ctxbk.lineWidth  = 1;
				this.ctxbk.moveTo(k,0);
				this.ctxbk.lineTo(k,sx.y);
				this.ctxbk.stroke();
	...
	drawDays:function(){
		...
		for(var k=0;k<sx.x;k+=this.options.style.dayWidth){
			this.ctxbk.beginPath();
			var dt=new Date(date);
			this.ctxbk.strokeStyle = "rgba(0, 0, 0,.065)";
		    this.ctxbk.lineWidth  = 1;
			this.ctxbk.moveTo(k,0);
			this.ctxbk.lineTo(k,sx.y);
			this.ctxbk.stroke();
	...
	drawTime:function(){
		...
		this.drawWeeks();
		this.drawMonths()
		this.ctxbk.fillStyle = "rgba(255, 255, 255,.7)";
		this.ctxbk.fillRect(0,0,sx.x,40);
		this.ctxbk.strokeStyle = "rgba(0, 0, 0,.3)";
	    this.ctxbk.lineWidth  = 1;
		this.ctxbk.moveTo(0,40);
		this.ctxbk.lineTo(sx.x,40);
		this.ctxbk.stroke();
		var lingrad2 = this.ctxbk.createLinearGradient(0,40,0,45);
  		lingrad2.addColorStop(0, 'rgba(0,0,0,.2)');
  		lingrad2.addColorStop(1, 'rgba(0,0,0,0)');
		this.ctxbk.fillStyle = lingrad2;
		this.ctxbk.fillRect(0,40,sx.x,5);
	},
	drawConnection:function(from,to){
		if(from){
			...
			var firstPosX=Math.round(((toPos.left+fromPos.right))/2);
			var firstPosY=Math.round(fromM);
			var secondPosX=Math.round(((toPos.left+fromPos.right))/2);
...
TimeLine.Project=new Class({
	...
	draw:function(){
		...
		var adjMinus=0;
		var today=new Date();
		var classN=this.project.due.getTime()>today.getTime()?"unknown":this.project.due.getTime()>=this.project.to.getTime()?"good":"bad"
		...
		}).injectInside(this.timeline.text);
		...
		this.timeline.ctxbk.beginPath();
		this.timeline.ctxbk.lineWidth  = 0;
		var lingrad2 = this.timeline.ctxbk.createLinearGradient(xin,yin,xin,yin+this.timeline.options.style.projectTitleEight);
		var bkCol="";
		switch (classN) {
			case "good":
		...
		this.timeline.ctxbk.strokeStyle = "rgba(255,255,255,1)";
		this.timeline.ctxbk.lineWidth  = 1;
		var tx=this.timeline.canvasbk.roundedRectangle(xin-4.5-118,yin-4.5,maxDatePos+9+118,rectH+9,10);
		this.timeline.ctxbk.stroke();
		this.timeline.ctxbk.strokeStyle = "rgba(0,0,0,.3)";
		this.timeline.ctxbk.lineWidth  = .5;
		this.timeline.ctxbk.fillStyle = bkCol;
		var tx=this.timeline.canvasbk.roundedRectangle(xin-4-118,yin-4,maxDatePos+8+118,rectH+8,10);
		this.timeline.ctxbk.fill();
		this.timeline.ctxbk.stroke();
		this.timeline.ctxbk.strokeStyle = "rgba(255, 255,255,1)";
		this.timeline.ctxbk.lineWidth  = 1;
		var tx=this.timeline.canvasbk.roundedRectangle(xin-.5,yin-.5,maxDatePos+1,this.timeline.options.style.projectTitleEight+1,10);
		this.timeline.ctxbk.stroke();
		this.timeline.ctxbk.strokeStyle = "rgba(0, 0, 0,.4)";
		this.timeline.ctxbk.lineWidth  = 1;
		this.timeline.ctxbk.fillStyle = lingrad2;
		var tx=this.timeline.canvasbk.roundedRectangle(xin,yin,maxDatePos,this.timeline.options.style.projectTitleEight,10);
		this.timeline.ctxbk.fill();
		this.timeline.ctxbk.stroke();
		this.timeline.ctxbk.fillStyle ='rgba(255,255,255,.5)';
		var tx=this.timeline.canvasbk.roundedRectangle(xin+1,yin+1,dueDatePos-2,this.timeline.options.style.projectTitleEight-2,10);
		this.timeline.ctxbk.fill();
		this.resources.each(function(resource){
			resource.draw();
		}.bind(this));
...
TimeLine.Resource=new Class({
	...
	draw:function(){
		...
		this.project.timeline.ctxbk.beginPath();
		this.project.timeline.ctxbk.strokeStyle = "rgba(0, 0, 0,.1)";
	    this.project.timeline.ctxbk.lineWidth  = 4;
		...
		this.project.timeline.ctxbk.moveTo(projL,projT+this.resource.position);
		var projW=projL+(this.project.box.getSize().size.x)-10;
		this.project.timeline.ctxbk.lineTo(projW,projT+this.resource.position);
		this.project.timeline.ctxbk.closePath();
		this.project.timeline.ctxbk.stroke();
		this.drawTasks();
	},
	...
	drawTasks:function(){
		...
		this.resource.tasks.each(function(task){
			var today=new Date();
			var classN=task.due.getTime()>today.getTime()?"unknown":task.due.getTime()>=task.to.getTime()?"good":"bad";
			...
			this.project.timeline.ctxbk.beginPath();
			this.project.timeline.ctxbk.lineWidth  = 0;
			...
			var lingrad2 = this.project.timeline.ctxbk.createLinearGradient(xin,yin,xin,yin+this.project.timeline.options.style.resourceHeight);
			switch (classN) {
				case "good":
			...
			this.project.timeline.ctxbk.strokeStyle = "rgba(255, 255,255,1)";
			this.project.timeline.ctxbk.lineWidth  = 1;
			var tx=this.project.timeline.canvasbk.roundedRectangle(xin-.5,yin-.5,maxDatePos+1,this.project.timeline.options.style.resourceHeight+1,10);
			this.project.timeline.ctxbk.stroke();
			this.project.timeline.ctxbk.strokeStyle = "rgba(0, 0, 0,.4)";
			this.project.timeline.ctxbk.lineWidth  = 1;
			this.project.timeline.ctxbk.fillStyle = lingrad2;
			var tx=this.project.timeline.canvasbk.roundedRectangle(xin,yin,maxDatePos,this.project.timeline.options.style.resourceHeight,10);
			this.project.timeline.ctxbk.fill();
			this.project.timeline.ctxbk.stroke();
			this.project.timeline.ctxbk.fillStyle ='rgba(255,255,255,.5)';
			var tx=this.project.timeline.canvasbk.roundedRectangle(xin+1,yin+1,dueDatePos-2,this.project.timeline.options.style.resourceHeight-2,10);
			this.project.timeline.ctxbk.fill();
			task.box=taskBox;
		}.bind(this));
	}
